...
export class ActivityAreaController {
  ...
  ): Promise<ActivityAreaResponseDto[] | PageDto<ActivityAreaResponseDto>> {
  ...
  @ApiResponse({
    status: 201,
    type: ActivityAreaResponseDto,
    description: 'Área de actividad creada exitosamente',
  })
  @ApiResponse({ status: 400, description: 'Datos de entrada inválidos' })
  async createActivityArea(
    @Body() createDto: CreateActivityAreaDto,
  ): Promise<ActivityAreaResponseDto> {
  ...
  @ApiResponse({
    status: 200,
    type: ActivityAreaResponseDto,
    description: 'Área de actividad actualizada exitosamente',
  })
  @ApiResponse({ status: 404, description: 'Área de actividad no encontrada' })
  @ApiResponse({ status: 400, description: 'Datos de entrada inválidos' })
  ...
  @ApiResponse({
    status: 400,
    description:
      'No se puede eliminar el área porque tiene sub-escenarios asociados',
  })
  async deleteActivityArea(
    ...
    return {
      success,
      message: success
        ? 'Área de actividad eliminada exitosamente'
        : 'No se pudo eliminar el área de actividad',
    };
  }
}
